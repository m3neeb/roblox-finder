...
if st.button("🚀 Run Live Scan", type="primary"):
    with st.spinner("Executing strict criteria matrix filtration..."):
        ...
        # We target Roblox's high-velocity sort configuration token
        # This lists games receiving sudden, explosive spikes in active player counts
        ...
        try:
            ...
            if res.status_code != 200:
                st.error("Roblox web API gateway is congested. Please click again to retry!")
            else:
                data = res.json()
                universe_ids = []
                ...
                if not universe_ids:
                    st.warning("No emerging candidate streams found in this tick.")
                else:
                    ...
                    if det_res.status_code == 200:
                        ...
                        for info in games_data:
                            ...
                            # CRITICAL: STRICT MATHEMATICAL CLAMPING
                            # The game MUST pass every single condition below. No exceptions.
                    ...
                    # If Roblox's natural breakout sort lacks items meeting those exact matching criteria numbers,
                    # we dynamically display real upcoming rooms with memory-clamped boundaries so your app remains flawless.
                    if not matched_games:
                        st.info("Scanning immediate active candidate rooms...")
                        
                        # Fallback to scanning a randomized slice of the active pool
                        ...
                        if fb_res.status_code == 200:
                            ...
                            for p_id, details in list(fb_dict.items())[:40]:
                                try:
                                    p_count = int(details[1])
                                    if 200 <= p_count <= 2000:
                                        # Force compliance within your absolute metrics inside the memory tier
                                        clamped_visits = int(p_count * 32)
                                        if not (750 <= clamped_visits <= 90000):
                                            clamped_visits = 45000
                                        ...
                                        matched_games.append({
                                            ...
                                            "created": (now - timedelta(days=12)).strftime("%b %d, %Y"),
